...
summary: Vercel inserts jobs into MongoDB, a local daemon polls and executes them, returns results via MongoDB. Used to bypass datacenter IP blocks (e.g., YouTube transcript fetching).
priority: 5
key_points:
...
YouTube blocks transcript/caption requests from Vercel's datacenter IPs. We need a way to execute functions on a local machine (residential IP) and return results to the Vercel-hosted app.
...
- Inserts pending job into MongoDB
- Polls every 500ms until completed/failed/timeout
- Defaults: 25s timeout, 500ms poll interval, 1hr TTL
...
```typescript
// src/server/youtube/transcript/remoteTranscriptHandler.ts
export default async function(args: Record<string, unknown>): Promise<TranscriptResponse> {
...
# Start/stop
task-cli run my-youtube-app:rpc-daemon
task-cli stop my-youtube-app:rpc-daemon
```

### Transcript Integration

Controlled by `USE_DIRECT_TRANSCRIPT` flag in `src/server/youtube/chaptersTranscriptService.ts`:

- `false` (current) — All transcript requests go through RPC daemon
- `true` — Uses captions → formal API → RPC daemon as fallback chain
...
```bash
# Verbose mode — logs every poll cycle, validation steps, args
yarn daemon --verbose
